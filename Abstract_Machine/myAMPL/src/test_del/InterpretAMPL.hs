...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
collect_handles:: [HANDLE_SPEC] -> SYM_TBL -> SYM_TBL
...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
collect_cohandles ::  [HANDLE_SPEC] -> SYM_TBL -> SYM_TBL
...
collect_constructors :: [STRUCTOR_SPEC] -> SYM_TBL -> SYM_TBL
...
collect_destructors :: [STRUCTOR_SPEC] -> SYM_TBL -> SYM_TBL
...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
collect_functions :: [FUNCTION_SPEC] -> SYM_TBL -> SYM_TBL
...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
collect_processes :: [PROCESS_SPEC] -> SYM_TBL -> SYM_TBL
...
look_up_sym ((str',entry):rest) str
                  | str==str' = case entry of 
                         S_FUN n ->     SYM_FUN n
...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
look_up_syms  :: SYM_TBL -> (String, String) -> SYM_VALUES

look_up_syms [] (s_1,s_2) = error ("Symbol <"++s_1++"."++s_2++"> not in scope.")
look_up_syms ((str,entry):rest) (s_1,s_2) 
        | str==s_1 = case entry of 
                      S_CONSTR constrs -> find_constr constrs (s_1,s_2)
                      S_DESTR destrs -> find_destr destrs (s_1,s_2)   
                      S_HAND hands -> find_hand hands (s_1,s_2)
                      S_CHAND hands -> find_chand hands (s_1,s_2)
                      _ -> error ("Symbol <"++s_1++"> is a function or process not a type.")
         | otherwise = look_up_syms rest (s_1,s_2) 

---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
look_up_syms_structs :: SYM_TBL -> (String,String) -> [String]
...
---------------------------------------------------------------------------------------
find_constr :: [(String,Int,Int)] -> (String,String) -> SYM_VALUES

find_constr [] (s_1,s_2)    =  error ("Symbol <"++s_1++"."++s_2++"> not in scope.")
find_constr ((s,n,m):rest) (s_1,s_2) 
      | s==s_2              = SYM_CONSTR n m
      | otherwise           = find_constr rest (s_1,s_2)

---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
find_destr :: [(String,Int,Int)] -> (String,String) -> SYM_VALUES

find_destr [] (s_1,s_2)     =  error ("Symbol <"++s_1++"."++s_2++"> not in scope.")
find_destr ((s,n,m):rest) (s_1,s_2) 
      | s==s_2              = SYM_DESTR n m
      | otherwise           = find_destr rest (s_1,s_2)

---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
find_hand :: [(String,Int)] -> (String,String) -> SYM_VALUES

find_hand [] (s_1,s_2)      =  error ("Symbol <"++s_1++"."++s_2++"> not in scope.")
find_hand ((s,n):rest) (s_1,s_2) 
      | s==s_2              = SYM_HANDLE n
      | otherwise           = find_hand rest (s_1,s_2)

---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
find_chand :: [(String,Int)] -> (String,String) -> SYM_VALUES

find_chand [] (s_1,s_2)     =  error ("Symbol <"++s_1++"."++s_2++"> not in scope.")
find_chand ((s,n):rest) (s_1,s_2) 
      | s==s_2              =  SYM_COHANDLE n
      | otherwise           =  find_chand rest (s_1,s_2)
...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
translate :: TRANSLATION -> String -> Int 
...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
translate_list :: TRANSLATION -> [String] -> [Int]
...
look_up_translation ((str',p,n):rest) str
                           | str'==str = (n,p)
...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
load_args :: [String] -> [String]  -> [AMPLCOM]

load_args stack args             = load_args1 [] args stack 

load_args1 :: [AMPLCOM] -> [String] -> [String] -> [AMPLCOM]
load_args1 ls [] stack           = ls
load_args1 ls (arg:args) stack   = load_args1 ((AMC_LOAD (depth_in_stack stack arg)):ls) args stack
